"""
Sea Level Rise Visualization for Germany
...
OUTPUT_VIDEO = os.path.join(SCRIPT_DIR, "sea_level_rise_germany.mp4")
...
def load_dem_data():
    """Load the clipped Germany DEM."""
    dem_path = os.path.join(DATA_DIR, "germany_dem_clipped.tif")
    if not os.path.exists(dem_path):
        print("ERROR: DEM data not found. Run download_data.py first!")
        sys.exit(1)
...
def load_water_bodies():
    ...
    water_raster_path = os.path.join(DATA_DIR, "water_bodies_raster.npy")
...
def draw_text_on_frame(frame_img, sea_level, fonts):
    ...
    for lv in [0, 200, 400, 600, 800, 1000]:
...
def main():
    print("=== Sea Level Rise Visualization - Germany (GPU) ===\n")
    ...
    dem_np, bounds = load_dem_data()

    print("Loading water bodies...")
    water_np = load_water_bodies()
