...
def robust_download(url, dest_path):
    """Download a file, falling back to PowerShell on Windows if urllib fails (SSL issues)."""
    try:
    ...
    for col, row in tile_coords:
        ...
        for url in urls:
            try:
                ...
                zip_tmp = os.path.join(tiles_dir, f"{tile_name}.zip")
                robust_download(url, zip_tmp)
